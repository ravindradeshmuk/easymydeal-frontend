...
import { Card, CardActionArea, CardContent, Typography, Grid,Box,Divider} from '@mui/material';
// import { red, blue, green, orange, deepPurple, lightBlue } from '@mui/material/colors';
import image4 from "../images/DTH middle image.jpg"
import { styled } from '@mui/system';
// Importing some generic icons as placeholders
import LiveTvIcon from '@mui/icons-material/LiveTv'; // Placeholder icon for DTH
import SettingsInputAntennaIcon from '@mui/icons-material/SettingsInputAntenna'; // Placeholder for other operators

// Import some icons for illustration purposes from 
const OperatorCard = styled(Card)({
...
const operators = [
   { name: 'Videocon D2H', icon: <LiveTvIcon />, path: '/videocon' },
  { name: 'Tata Play', icon: <LiveTvIcon />, path: '/tata-play' },
  { name: 'Dish TV', icon: <SettingsInputAntennaIcon />, path: '/dish-tv' },
  { name: 'Airtel DTH', icon: <LiveTvIcon />, path: '/airtel-dth' },
  { name: 'Sun Direct TV', icon: <SettingsInputAntennaIcon />, path: '/sun-direct-tv' },

   ]

const OperatorSelectionElectricity= () => {
  const navigate = useNavigate();

  const handleCardClick = (path) => {
    // Navigate to the path associated with the operator
    navigate(path);
  };
    ...
    <>
      ...
      <Grid container spacing={2} justifyContent="center">
        {operators.map((operator) => (
          <Grid item key={operator.name}>
            <OperatorCard>
              <CardActionArea onClick={() => handleCardClick(operator.path)}>
                <CardContent>
                  <img src={operator.logo} alt={operator.name} style={{ marginBottom: '8px' }} />
      ...
      <Box>
        ...
        <Grid item xs={12} md={6}>
            <ResponsiveImage  src={image4} alt="Mobile Recharge" />
          </Grid>
        <Typography margin='20px' gutterBottom
          sx={{
            fontWeight: 'bold', // Makes the text bolder
...
export default OperatorSelectionElectricity;
